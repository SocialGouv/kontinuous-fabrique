...
module.exports = async (values, _options, {config, utils, ctx})=>{
  ...
  const {
    FABRIQUE_BASE_DOMAIN,
    FABRIQUE_RANCHER_PROJECT_ID,
    FABRIQUE_JOB_NAMESPACE
  } = processEnv
  ...
  let domain
  if(FABRIQUE_BASE_DOMAIN){
    domain = FABRIQUE_BASE_DOMAIN
  } else {
    domain = isProd ? defaultRootDomain : `dev.${defaultRootDomain}`
  }

  const rancherProjectId = FABRIQUE_RANCHER_PROJECT_ID
  ...
  const jobNamespace = FABRIQUE_JOB_NAMESPACE || `${repositoryName}-ci`
